...
def run_guided_inference(conf, original_contigs: List[str], sampled_contigs_list: List[List[str]], custom_reward_fn=None, profiler=None):
    ...
    for run_id in range(1, n_runs + 1):
        ...
        # Time the complete per-run inference path, including sampler construction.
        # ``perf_counter`` supplies a monotonic duration while the UTC timestamps
        # make the saved results auditable across machines and time zones.
        ...
        # Add run and timing columns. Each row generated by a run shares these
        # values because the timing applies to the complete particle batch.
        run_results_df['run'] = run_id
        run_results_df['run_started_at_utc'] = run_started_at_utc
        run_results_df['run_completed_at_utc'] = run_completed_at_utc
        run_results_df['run_elapsed_seconds'] = run_elapsed_seconds
        ...
        print(f"Completed run {run_id}/{n_runs} in {run_elapsed_seconds:.1f} seconds")
