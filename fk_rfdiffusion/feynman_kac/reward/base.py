...
def _run_proteinmpnn(
    ...
    with open(seq_files[0], 'r') as f:
        ...
        for i, line in enumerate(lines):
            # ProteinMPNN writes the input/reference sequence first, followed
            # by the sampled designs under ``>T=...`` headers.  Only the latter
            # are candidates to thread back onto the structure.
